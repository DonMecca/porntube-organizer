...
/**
 * Wall-clock ceiling for filling one catalog page, threaded through every paging
 * loop. MAX_UPSTREAM_PAGES bounds the page COUNT, not the time: 40 pages at the
 * 12s timeout below is up to 8 minutes, and the studio search walks three query
 * variants on top of that, so a single catalog request could stall far longer than
 * any Stremio client will wait. Returning a short page beats hanging the client.
 */
const FILL_BUDGET_MS = Number(process.env.PORNTUBE_FILL_BUDGET_MS) || 20000;

function budgetExpired(startedAt) {
  return Date.now() - startedAt > FILL_BUDGET_MS;
}
 ...
 */
async function pagedSearchFiltered(configB64, catalogId, query, genre, skipN, startedAt = Date.now()) {
  const matched = [];
  const seen = new Set();
  let upstreamSkip = 0;
  let pages = 0;
  ...
  while (pages < MAX_UPSTREAM_PAGES && matched.length < skipN + PAGE_SIZE) {
    // Stop as soon as the caller's budget is spent; see FILL_BUDGET_MS.
    if (budgetExpired(startedAt)) {
      console.warn(`[upstream] Fill budget spent after ${pages} page(s); returning a short page`);
      break;
    }
    pages += 1;
    let metas = [];
...
async function organizedVrCatalog(configB64, genre, skip = 0) {
  ...
  // One budget for the whole request, shared by every paging loop below.
  ...
  for (const q of searchOrder.slice(0, 3)) {
    ...
    const page = await pagedSearchFiltered(configB64, 'pt_vr', q, genre, skipN, startedAt);
...
async function organizedOldCatalog(configB64, genre, skip = 0) {
  ...
  for (const q of studioQueryVariants(genre).slice(0, 2)) {
    if (budgetExpired(startedAt)) break;
    const page = await pagedSearchFiltered(configB64, 'pt_old', q, genre, skipN, startedAt);
    if (page.length) return { metas: page };
  }
...
/**
 * Upstream catalogue health (verified 2026-09-22 against ptube.ers.pw v0.5.3):
 *
 *   pt_vr   - healthy: ~36 results per studio, paginates correctly
 *   pt_old  - effectively empty: 2 items for the one studio that has any, 0 for
 *             every other studio tested, and it ignores `skip` entirely. This is
 *             an upstream data gap, not an addon bug, so `pt_old_org` legitimately
 *             shows very little. It is left enabled because it degrades cleanly
 *             (an empty page, not an error) and would populate if upstream adds data.
 *   tpdb    - healthy: richest provider, and the only one still listed in the
 *             upstream manifest.
 *
 * Both pt_vr and pt_old are no longer advertised in the upstream manifest, so they
 * are an undocumented dependency: if upstream removes the paths, 2 of the 3
 * catalogs break silently.
 */
